...
2. `docs/systems/architecture/ARCHITECTURE-simple.md` (standard for all agents)
...
If patterns load, review and apply relevant ones. Track which you use.
...
# - Comment to claim: "Starting Context Pack work."
```
...
|---------------------|----------------|---------------------|
<CONFIG>Component type paths</CONFIG>
...
cat docs/systems/architecture/ARCHITECTURE.md
cat docs/systems/architecture/LAYERS.md
```

Focus on:
- Non-negotiables (checklist for any work)
- Layering and dependency rules (import boundaries)
- Artifact/receipt/manifest constraints (use `docs/systems/architecture/INDEX.md` to route to the right subdocs)
...
**These constraints apply to ALL work.** The Context Pack template (Section 3) has subsections for each type of constraint with the rules pre-filled. Your job is to:

1. **Read the architecture docs** to understand which rules apply:
   - `docs/systems/architecture/ARCHITECTURE-simple.md` — Non-negotiables checklist
   - `docs/systems/architecture/COMPONENT-TYPES.md` — Component-specific requirements matrix
   - `docs/systems/architecture/LAYERS.md` — Import boundary rules
   - `docs/systems/architecture/SECURITY.md` — Path validation rules (9 required)
   - `docs/systems/architecture/QUALITY-ATTRIBUTES.md` — Test formula, error handling

2. **Fill in Section 3 of the template:**
   - 3.1: Set the component type tag
   - 3.2: Extract the specific forbidden imports for this component type
   - 3.3: Mark which requirements apply (artifact contracts, async patterns, etc.)
   - 3.4-3.9: Keep or delete based on whether they apply (the rules are pre-filled)
   - 3.10: Add any other constraints from ARTIFACTS.md, EXECUTION-MODEL.md, etc.

**CRITICAL:** The spec creator will NOT read architecture docs. The template has the rules pre-filled so you just need to mark which apply and fill in component-specific details (like the exact forbidden imports for this type).
...
cat docs/support/ops/INDEX.md            # Runbooks, procedures (ops uses INDEX.md)
...
Use <CONFIG>Research tool</CONFIG> to ask targeted questions about specific concerns — gotchas, edge cases, version issues, library combination problems. Label findings as "(Source: Research)" in the Context Pack.
...
3. **Check incident history** (if relevant):
   - `docs/support/incidents/` — Has this type of problem bitten us before?
...
Based on component type, research additional areas per `docs/systems/architecture/COMPONENT-TYPES.md`.

Key areas by type:
<CONFIG>Component type research areas - generated by /falcon-config</CONFIG>

### IF [TYPE: ux]

Research React/TypeScript patterns:
- Existing component patterns in `<CONFIG>UX source directory</CONFIG>`
...
**Verbalize your reasoning** — When you make decisions or solve problems, explain your thinking. This enables pattern extraction for future tasks.
...
### From Internal Docs
- **Known issues:** <from docs/support/incidents/ or docs/troubleshooting/>
- **Existing patterns:** <gotchas noted in code comments>

<CONFIG>Component-specific requirements</CONFIG>
```
...
```bash
# Use /linear-tool skill for Linear operations
# - Create document: "CON-XXX Context Pack: <short title>" with content from /tmp/context-pack-CON-XXX.md in <project-name>
```
...
# - Add label: has_context_pack
# - Swap labels (remove agent_working, add agent_ready)
# - Comment with Context Pack URL (REQUIRED format): "Context Pack complete. Context Pack: <URL>. Summary: <1-2 sentences>. Open questions (BLOCKER): <list or 'None'>. Ready for review. Next steps: Run /checkout CON-XXX to review the context pack."
```
...
```bash
# Use /linear-tool skill for Linear operations
# - Add label: human_input
# - Comment: "Context Pack blocked. Blockers requiring human decision: 1. <blocker 1> 2. <blocker 2>. Please resolve these questions before spec work can begin."
```
