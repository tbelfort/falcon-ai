...
| Question | If YES, action needed |
|----------|----------------------|
| Do changes add new API endpoints or services? | Add to `docs/systems/apps/<app>/api/` |
| Do changes add new environment variables or config? | Add to `docs/systems/config/` |
| Do changes add integrations with external services? | Add to `docs/support/integrations/` |
| Do changes introduce common failure modes or gotchas? | Add to `docs/support/troubleshooting/` |
| Do changes add new CLI tools or scripts? | Add to `docs/support/ops/` (runbook or usage guide) |
| Do changes add new database tables or schemas? | Add to `docs/systems/apps/<app>/dbs/` |
| Do changes add new workflows or processes? | Add to `docs/support/ops/` or relevant README |
| Are there non-obvious behaviors that future developers will need to understand? | Add to `docs/design/` or `docs/systems/architecture/` |
