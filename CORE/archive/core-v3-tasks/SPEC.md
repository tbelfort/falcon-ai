...
This determines which conditional requirements apply throughout spec writing.
...
- `docs/architecture/ARCHITECTURE-simple.md`
...
Use the template at `.claude/CORE/TEMPLATES/gemini/ai-docs-research.md`.
...
- `packages/foundry/foundry_core/specs/`
- `packages/forge/forge_web/specs/`
- `apps/intent-atlas/specs/`
...
### Step 4.1: Mandatory Sections (ALL SPECS)

Every spec MUST include these sections regardless of component type:

#### Import Boundaries Section (MANDATORY)

```markdown
## Import Boundaries

**Allowed imports:**
- <list packages this component may import>

**Forbidden imports:**
- <list packages this component must NOT import>

**Test requirement:** Integration test verifying no forbidden imports.
```

#### Error Handling Section (MANDATORY)

```markdown
## Error Handling

**Error types used:**
- <list FoundryError subclasses or new error types>

**Error context requirements:**
- <what context must be included in errors>

**Prohibited patterns:**
- Bare `except Exception`
- Swallowing errors without logging
- Returning None on error (use Result type or raise)
```

---

### Step 4.2: Conditional Requirements

#### IF [TYPE: foundry-package] or [TYPE: forge-stage] - Pydantic Models

For ANY Pydantic model with `frozen=True`:

```markdown
## Model Immutability

| Model | Collection Fields | Type Used | Correct? |
|-------|-------------------|-----------|----------|
| <name> | inputs | tuple[X, ...] | Yes |
| <name> | items | list[X] | NO - must be tuple |

**Rule:** Frozen models MUST use immutable collection types:
- `tuple[X, ...]` not `list[X]`
- `frozenset[X]` not `set[X]`
```

#### IF [TYPE: forge-stage] - Artifact Requirements

```markdown
## Artifact Contract

**Inputs (artifacts read):**
| Path Pattern | Kind | Required? |
|--------------|------|-----------|

**Outputs (artifacts written):**
| Path Pattern | Kind | Contains Manifest? |
|--------------|------|-------------------|

**Side Effects:**
| Effect | Receipt Path | Receipt Schema |
|--------|--------------|----------------|

**Validation rules:**
- All IO goes through ArtifactStore (no direct file access)
- Side effects MUST write receipts
- Multi-file outputs MUST use manifests with base_prefix containment
```

#### IF [TYPE: worker/engine/weaver] - Async Requirements

```markdown
## Async Behavior

**Async functions:**
| Function | Timeout | Cancellation Handling |
|----------|---------|----------------------|

**Blocking operations:**
- <list any sync operations and how they're handled>
```

#### IF [TYPE: ux] - React/TypeScript Requirements

```markdown
## Component Specification

**Components:**
| Component | Props | State | Hooks Used |
|-----------|-------|-------|------------|
...
### Step 4.3: Cross-Field Validation (IF PYDANTIC MODELS)

For models with related fields, specify cross-field validators:

#### Cross-Field Validation Table

| Model | Fields | Constraint | Validator Type |
|-------|--------|------------|----------------|
| StageSpec | inputs, outputs | No path collision | model_validator |
| StageSpec | inputs | No duplicate paths | model_validator |
| ReceiptSchema | status, error | If status='error', error MUST NOT be None | model_validator |

**Rule:** Every relationship between fields needs a validator.

---

### Step 4.4: Validation Strictness (ALL SPECS)

**Default: STRICT.** Document any relaxation with justification.

#### Strictness Decisions Table

| Validation Point | Strict Behavior (DEFAULT) | Relaxed Behavior | This Spec |
|------------------|---------------------------|------------------|-----------|
| Error states | Require error field when status='error' | Allow None | STRICT |
| Path whitespace | Reject leading/trailing/only whitespace | Allow/strip | STRICT |
| Duplicate entries | Reject duplicates in collections | Allow | STRICT |
| Missing trailing / | Reject with error | Auto-add | STRICT |
| Double slashes | Reject `//` in paths | Allow | STRICT |

**If choosing RELAXED, document justification.**

---

### Step 4.5: Path Security (IF HANDLING ARTIFACT PATHS)

When spec handles artifact paths, MUST include ALL validations:

#### Path Validation Requirements

| # | Validation | Rule | Test Required |
|---|------------|------|---------------|
| 1 | Absolute path | Reject paths starting with "/" | YES |
| 2 | Parent traversal | Reject paths with ".." | YES |
| 3 | Backslash | Reject paths with "\\" | YES |
| 4 | Empty segments | Reject paths with "//" | YES |
| 5 | Leading whitespace | Reject paths starting with whitespace | YES |
| 6 | Trailing whitespace | Reject paths ending with whitespace | YES |
| 7 | Whitespace only | Reject whitespace-only paths | YES |
| 8 | Null bytes | Reject paths containing "\\x00" | YES |
| 9 | Empty string | Reject empty paths | YES |

**All 9 validations are REQUIRED. Include test cases for each.**

---

### Step 4.6: Testing Section - Test Count Formula

Use this formula to calculate the minimum number of tests:
...
**Add for each factor present:**
| Factor | Points | This Spec |
|--------|--------|-----------|
| Pydantic model | +5 each | ___ x 5 = ___ |
| Cross-field validator | +3 each | ___ x 3 = ___ |
| External dependency | +3 each | ___ x 3 = ___ |
| Async function | +2 each | ___ x 2 = ___ |
| Path/artifact handling | +10 | ___ |
| Side effects | +5 | ___ |
| **TOTAL MINIMUM** | | ___ |
...
- [ ] Import boundary test (verifies no forbidden imports)
- [ ] Error handling tests (each error path)

**Conditional:**
- [ ] Path security tests - 9 tests minimum (IF path handling)
- [ ] Async behavior tests (IF async)
- [ ] Artifact contract tests (IF forge-stage)
- [ ] Component render tests (IF ux)
- [ ] Immutability tests (IF frozen Pydantic models)
- [ ] Cross-field validation tests (IF cross-field validators)
```
...
**Spec:** <full path to spec, e.g., packages/foundry/foundry_core/specs/artifact-store.md>
