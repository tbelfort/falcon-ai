...
When launching scouts, include component type from the spec or code location.

See `docs/architecture/COMPONENT-TYPES.md` for:
- Type definitions and location patterns
- Type-specific requirements matrix
- What each scout should focus on per type
...
Per `docs/architecture/LAYERS.md` and `docs/architecture/COMPONENT-TYPES.md`, verify no forbidden imports exist.

Use grep to check for forbidden imports based on component type.

**Any forbidden import found -> BLOCKING**

### Immutability Check (if Pydantic models)

Per `docs/architecture/COMPONENT-TYPES.md#pydantic-model-requirements`, verify frozen models use immutable collection types.
