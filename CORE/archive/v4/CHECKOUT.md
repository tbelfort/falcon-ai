...
| Todo | Read `CORE/TASKS/CONTEXT_PACK.md` |
| Context Pack In Progress | **FAIL** — "Another agent is working on the context pack." |
| Context Pack In Review | Check labels (see below) |
| Ready for Spec | Read `CORE/TASKS/SPEC.md` |
| Spec In Progress | **FAIL** — "Another agent is working on the spec." |
| Spec Drafted | Read `CORE/TASKS/SPEC_HARDENING_TESTS.md` |
| Spec - Hardening Tests | **FAIL** — "Another agent is hardening the spec tests." |
| Spec In Review | Read `CORE/TASKS/SPEC_REVIEW.md` |
| Ready to Start | Read `CORE/TASKS/IMPLEMENT.md` |
| Work Started | Read `CORE/TASKS/IMPLEMENT.md` |
| In Review | Check labels and override (see below) |
| Review Passed | Read `CORE/TASKS/TESTING.md` |
| Testing | **FAIL** — "Another agent is running tests." |
| Ready to Merge | Read `CORE/TASKS/MERGE.md` |

### Context Pack In Review — Label Check

When issue is in **Context Pack In Review**, check for the improvement label:

1. **If `needs_improve_context_pack` label exists:** Read `CORE/TASKS/CONTEXT_PACK_IMPROVE.md`
2. **Otherwise (no label):** Read `CORE/TASKS/CONTEXT_PACK_REVIEW.md`
...
1. **If `--fix` override provided:** Read `CORE/TASKS/FIX_REVIEW.md`
2. **If `--review` override provided:** Read `CORE/TASKS/PR_REVIEW.md`
3. **If `code-review-failed` label exists:** Read `CORE/TASKS/FIX_REVIEW.md`
4. **If `spec-review-fail` label exists:** Read `CORE/TASKS/FIX_REVIEW.md` (spec compliance issues)
5. **Otherwise (no label, no override):** Read `CORE/TASKS/PR_REVIEW.md`
